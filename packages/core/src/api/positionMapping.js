 ...
 */
side = "left") {
    const ySyncPluginState = ySyncPluginKey.getState(editor._tiptapEditor.state);
    if (!ySyncPluginState) {
    ...
    position + (side === "right" ? 1 : 0), ySyncPluginState.binding.type, ySyncPluginState.binding.mapping);
    return () => {
        const curYSyncPluginState = ySyncPluginKey.getState(editor._tiptapEditor.state);
        const pos = relativePositionToAbsolutePosition(curYSyncPluginState.doc, curYSyncPluginState.binding.type, relativePosition, curYSyncPluginState.binding.mapping);

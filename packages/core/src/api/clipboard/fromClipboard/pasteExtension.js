...
function defaultPasteHandler({ event, editor, prioritizeMarkdownOverHTML, plainTextAsMarkdown, }) {
    // Special case for code blocks, as they do not support any rich text
    // formatting, so we force pasting plain text.
    const isInCodeBlock = editor.transact((tr) => tr.selection.$from.parent.type.spec.code &&
...
export const createPasteFromClipboardExtension = (editor, pasteHandler) => Extension.create({
                    ...
                    handleDOMEvents: {
                        paste(_view, event) {
                            ...
                            return pasteHandler({
                                event,
                                editor,
                                defaultPasteHandler: ({ prioritizeMarkdownOverHTML = true, plainTextAsMarkdown = true, } = {}) => {

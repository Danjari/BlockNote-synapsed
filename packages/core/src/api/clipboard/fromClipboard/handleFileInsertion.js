...
function insertOrUpdateBlock(editor, referenceBlock, newBlock, placement = "after") {
    let insertedBlockId;
    if (Array.isArray(referenceBlock.content) &&
        referenceBlock.content.length === 0) {
        insertedBlockId = editor.updateBlock(referenceBlock, newBlock).id;
    }
    else {
        insertedBlockId = editor.insertBlocks([newBlock], referenceBlock, placement)[0].id;
    }
        ...
        if (file) {
            ...
            if (event.type === "paste") {
                ...
                insertedBlockId = editor.transact((tr) => {
                    ...
                    const blockElement = editor.prosemirrorView?.dom.querySelector(`[data-id="${posInfo.node.attrs.id}"]`);
                    const blockRect = blockElement?.getBoundingClientRect();
                    return insertOrUpdateBlock(editor, editor.getBlock(posInfo.node.attrs.id), fileBlock, blockRect && (blockRect.top + blockRect.bottom) / 2 > coords.top
                        ? "before"
                        : "after");
                });
            }

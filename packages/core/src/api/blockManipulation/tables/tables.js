import { isPartialLinkInlineContent, isStyledTextInlineContent, } from "../../../schema/index.js";
import { getColspan, getRowspan, isPartialTableCell, mapTableCell, } from "../../../util/table.js";
/**
 ...
 */
export function getTableCellOccupancyGrid(block) {
            ...
            for (let i = startRow; i < startRow + rowspan; i++) {
                for (let j = startCol; j < startCol + colspan; j++) {
                    if (grid[i][j]) {
                        ...
                        throw new Error(`Unable to create occupancy grid for table, cell at ${i},${j} is already occupied`);
                    }
                    grid[i][j] = {
                        row,
                        col,
 ...
 */
export function getTableRowsFromOccupancyGrid(occupancyGrid) {
...
occupancyGrid = getTableCellOccupancyGrid(block)) {
    for (let r = 0; r < occupancyGrid.length; r++) {
        for (let c = 0; c < occupancyGrid[r].length; c++) {
            ...
            if (cell.row === relativeCellIndices.row &&
                cell.col === relativeCellIndices.col) {
                return { row: r, col: c, cell: cell.cell };
            }
        }
    }
    throw new Error(`Unable to resolve relative table cell indices for table, cell at ${relativeCellIndices.row},${relativeCellIndices.col} is not occupied`);
}
/**
 ...
 */
export function getDimensionsOfTable(block) {
 ...
 */
occupancyGrid = getTableCellOccupancyGrid(block)) {
    const occupancyCell = occupancyGrid[absoluteCellIndices.row]?.[absoluteCellIndices.col];
 ...
 */
export function getCellsAtRowHandle(block, relativeRowIndex) {
    ...
    const cells = new Array(occupancyGrid[0].length)
        .fill(false)
        .map((_v, col) => {
        return getRelativeTableCells({ row: absoluteRow, col }, block, occupancyGrid);
    })
        .filter((a) => a !== undefined);
    // Filter out duplicates based on row and col properties
    return cells.filter((cell, index) => {
        return (cells.findIndex((c) => c.row === cell.row && c.col === cell.col) === index);
    });
}
/**
 ...
 */
export function getCellsAtColumnHandle(block, relativeColumnIndex) {
    const occupancyGrid = getTableCellOccupancyGrid(block);
    ...
    const cells = new Array(occupancyGrid.length)
        .fill(false)
        .map((_v, row) => {
        return getRelativeTableCells({ row, col: absoluteCol }, block, occupancyGrid);
    })
        .filter((a) => a !== undefined);
    // Filter out duplicates based on row and col properties
    return cells.filter((cell, index) => {
 ...
 */
export function moveColumn(block, fromColIndex, toColIndex, occupancyGrid = getTableCellOccupancyGrid(block)) {
 ...
 */
export function moveRow(block, fromRowIndex, toRowIndex, occupancyGrid = getTableCellOccupancyGrid(block)) {
    // To move cells in a column, we need to layout the whole table
    // and then move the cells accordingly.
...
function isCellEmpty(cell) {
    ...
    if (isPartialTableCell(cell)) {
        ...
        return cell.every((c) => typeof c === "string"
            ? c.length === 0
 ...
 * @note This is a destructive operation, it will modify the {@link OccupancyGrid} in place.
 */
export function cropEmptyRowsOrColumns(block, removeEmpty, occupancyGrid = getTableCellOccupancyGrid(block)) {
    if (removeEmpty === "columns") {
        // strips empty columns on the right
        let emptyColsOnRight = 0;
        for (let cellIndex = occupancyGrid[0].length - 1; cellIndex >= 0; cellIndex--) {
            const isEmpty = occupancyGrid.every((row) => isCellEmpty(row[cellIndex].cell) && row[cellIndex].colspan === 1);
            if (!isEmpty) {
                break;
            }
            emptyColsOnRight++;
        ...
        for (let i = occupancyGrid.length - 1; i >= 0; i--) {
            ...
            const cellsToRemove = Math.max(occupancyGrid[i].length - emptyColsOnRight, 1);
    ...
    for (let rowIndex = occupancyGrid.length - 1; rowIndex >= 0; rowIndex--) {
        const isEmpty = occupancyGrid[rowIndex].every((cell) => isCellEmpty(cell.cell) && cell.rowspan === 1);
        if (!isEmpty) {
            break;
        }
        emptyRowsOnBottom++;
 ...
 * @note This is a destructive operation, it will modify the {@link OccupancyGrid} in place.
 */
export function addRowsOrColumns(block, addType, 
/**
 * The number of rows or columns to add.
 *
 * @note if negative, it will remove rows or columns.
 */
numToAdd, occupancyGrid = getTableCellOccupancyGrid(block)) {
 ...
 */
export function canRowBeDraggedInto(block, draggingIndex, targetRowIndex) {
 ...
 */
export function canColumnBeDraggedInto(block, draggingIndex, targetColumnIndex) {
 ...
 */
export function areInSameColumn(from, to, block) {

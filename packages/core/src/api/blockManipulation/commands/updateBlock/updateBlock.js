...
export const updateBlockCommand = (posBeforeBlock, block) => {
    return ({ tr, dispatch, }) => {
        if (dispatch) {
...
export function updateBlockTr(tr, posBeforeBlock, block, replaceFromPos, replaceToPos) {
    const blockInfo = getBlockInfoFromResolvedPos(tr.doc.resolve(posBeforeBlock));
    const pmSchema = getPmSchema(tr);
    if (replaceFromPos !== undefined &&
    ...
    if (blockInfo.isBlockContainer && newNodeType.isInGroup("blockContent")) {
        const replaceFromOffset = replaceFromPos !== undefined &&
            ...
            : undefined;
        const replaceToOffset = replaceToPos !== undefined &&
        ...
        updateBlockContentNode(block, tr, oldNodeType, newNodeType, blockInfo, replaceFromOffset, replaceToOffset);
    }
        ...
        tr.replaceWith(blockInfo.bnBlock.beforePos, blockInfo.bnBlock.afterPos, blockToNode({
...
function updateBlockContentNode(block, tr, oldNodeType, newNodeType, blockInfo, replaceFromOffset, replaceToOffset) {
    const pmSchema = getPmSchema(tr);
    let content = "keep";
    ...
    if (content === "keep") {
        ...
        const start = blockInfo.blockContent.beforePos + 1 + (replaceFromOffset ?? 0);
        const end = blockInfo.blockContent.beforePos +
            1 +
        ...
        tr.replace(start, end, new Slice(Fragment.from(content), startDepth - contentDepth - 1, endDepth - contentDepth - 1));
    }
    else {
        // use replaceWith to replace the content and the block itself
        // also reset the selection since replacing the block content
        // sets it to the next block.
        tr.replaceWith(blockInfo.blockContent.beforePos, blockInfo.blockContent.afterPos, newNodeType.createChecked({
            ...blockInfo.blockContent.node.attrs,
            ...block.props,
        }, content));
    }
}
function updateChildren(block, tr, blockInfo) {
    const pmSchema = getPmSchema(tr);
    if (block.children !== undefined && block.children.length > 0) {
        ...
        if (blockInfo.childContainer) {
            ...
            tr.step(new ReplaceStep(blockInfo.childContainer.beforePos + 1, blockInfo.childContainer.afterPos - 1, new Slice(Fragment.from(childNodes), 0, 0)));
        }
        else {
            ...
            tr.insert(blockInfo.blockContent.afterPos, pmSchema.nodes["blockGroup"].createChecked({}, childNodes));
        }
    }
}
export function updateBlock(tr, blockToUpdate, update, replaceFromPos, replaceToPos) {

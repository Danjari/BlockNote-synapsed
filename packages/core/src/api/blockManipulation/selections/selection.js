...
import { nodeToBlock, prosemirrorSliceToSlicedBlocks, } from "../../nodeConversions/nodeToBlock.js";
import { getNodeById } from "../../nodeUtil.js";
import { getBlockNoteSchema, getPmSchema } from "../../pmUtil.js";
export function getSelection(tr) {
    const pmSchema = getPmSchema(tr);
    // Return undefined if the selection is collapsed or a node is selected.
    if (tr.selection.empty || "node" in tr.selection) {
        return undefined;
    }
    const $startBlockBeforePos = tr.doc.resolve(getNearestBlockPos(tr.doc, tr.selection.from).posBeforeNode);
    const $endBlockBeforePos = tr.doc.resolve(getNearestBlockPos(tr.doc, tr.selection.to).posBeforeNode);
    ...
    if (blocks.length === 0) {
        throw new Error(`Error getting selection - selection doesn't span any blocks (${tr.selection})`);
    }
    return {
        blocks,
    };
}
export function setSelection(tr, startBlock, endBlock) {
    ...
    const anchorBlockConfig = schema.blockSchema[anchorBlockInfo.blockNoteType];
    const headBlockConfig = schema.blockSchema[headBlockInfo.blockNoteType];
...
export function getSelectionCutBlocks(tr) {
    ...
    const selectionInfo = prosemirrorSliceToSlicedBlocks(tr.doc.slice(start.pos, end.pos, true), pmSchema);
    return {
        _meta: {
            startPos: start.pos,
            endPos: end.pos,

    ...
    contentNode.content.forEach((rowNode, _offset, rowIndex) => {
        ...
        if (rowIndex === 0) {
            rowNode.content.forEach((cellNode) => {
                let colWidth = cellNode.attrs.colwidth;
        ...
        row.cells = rowNode.content.content.map((cellNode, cellIndex) => {
            ...
            const content = cellNode.content.content
                .map((child) => contentNodeToInlineContent(child, inlineContentSchema, styleSchema))
            ...
            if (isStyledTextInlineContent(currentContent)) {
                ...
                if (linkMark) {
                    ...
                    if (currentContent.href === linkMark.attrs.href) {
                        // Styles are the same.
                        if (JSON.stringify(currentContent.content[currentContent.content.length - 1]
 ...
 * TODO: test changes
 */
export function nodeToBlock(node, schema, blockSchema = getBlockSchema(schema), inlineContentSchema = getInlineContentSchema(schema), styleSchema = getStyleSchema(schema), blockCache = getBlockCache(schema)) {
    ...
    blockInfo.childContainer?.node.forEach((child) => {
        children.push(nodeToBlock(child, schema, blockSchema, inlineContentSchema, styleSchema, blockCache));
    });
    let content;
 ...
 */
export function docToBlocks(doc, schema, blockSchema = getBlockSchema(schema), inlineContentSchema = getInlineContentSchema(schema), styleSchema = getStyleSchema(schema), blockCache = getBlockCache(schema)) {
    const blocks = [];
    doc.firstChild.descendants((node) => {
        blocks.push(nodeToBlock(node, schema, blockSchema, inlineContentSchema, styleSchema, blockCache));
        return false;
    });
    return blocks;
}
 ...
 *
 */
export function prosemirrorSliceToSlicedBlocks(slice, schema, blockSchema = getBlockSchema(schema), inlineContentSchema = getInlineContentSchema(schema), styleSchema = getStyleSchema(schema), blockCache = getBlockCache(schema)) {
    // console.log(JSON.stringify(slice.toJSON()));
    function processNode(node, openStart, openEnd) {
        ...
        node.forEach((blockContainer, _offset, index) => {
            ...
            if (blockContainer.childCount === 0 || blockContainer.childCount > 2) {
                throw new Error("unexpected, blockContainer.childCount: " + blockContainer.childCount);
            }
            ...
            if (blockContainer.firstChild.type.name === "blockGroup") {
                ...
                const ret = processNode(blockContainer.firstChild, Math.max(0, openStart - 1), isLastBlock ? Math.max(0, openEnd - 1) : 0);
            ...
            const block = nodeToBlock(blockContainer, schema, blockSchema, inlineContentSchema, styleSchema, blockCache);
            const childGroup = blockContainer.childCount > 1 ? blockContainer.child(1) : undefined;
            let childBlocks = [];
            if (childGroup) {
                const ret = processNode(childGroup, 0, // TODO: can this be anything other than 0?
    ...
    if (slice.content.childCount !== 1) {
        throw new Error("slice must be a single block, did you forget includeParents=true?");
    }
    return processNode(slice.content.firstChild, Math.max(slice.openStart - 1, 0), Math.max(slice.openEnd - 1, 0));
}

...
function getBlockFromMousePos(mousePos, view) {
    ...
    const coords = {
        ...
        left: Math.min(Math.max(editorBoundingBox.left + 10, mousePos.x), editorBoundingBox.right - 10),
        top: mousePos.y,
    };
...
export class SideMenuView {
    ...
    constructor(editor, pmView, emitUpdate) {
        ...
        this.pmView.root.addEventListener("dragstart", this.onDragStart);
        this.pmView.root.addEventListener("dragover", this.onDragOver);
        this.pmView.root.addEventListener("drop", this.onDrop, true);
        this.pmView.root.addEventListener("dragend", this.onDragEnd, true);
     ...
     */
    findClosestEditorElement = (coords) => {
        ...
        editors.forEach((editor) => {
            ...
            const distanceX = coords.clientX < rect.left
                ? rect.left - coords.clientX
                : coords.clientX > rect.right
                    ? coords.clientX - rect.right
                    : 0;
            const distanceY = coords.clientY < rect.top
    ...
    onDragOver = (event) => {
        ...
        if (dragEventContext.isDropPoint &&
     ...
     */
    getDragEventContext = (event) => {
        // We need to check if there is text content that is being dragged (select some text & just drag it)
        const textContentIsBeingDragged = !event.dataTransfer?.types.includes("blocknote/html") &&
            !!this.pmView.dragging;
        ...
        const isDropWithinEditorBounds = isDropPoint && closestEditor.distance === 0;
    ...
    onDrop = (event) => {
        ...
        if (isDropPoint) {
            ...
            // selection.
            this.pmView.dispatch(this.pmView.state.tr.setSelection(TextSelection.create(this.pmView.state.tr.doc, this.pmView.state.tr.selection.anchor)));
            return;
        }
    ...
    dispatchSyntheticEvent(event) {
        ...
        const dropPointBoundingBox = this.pmView.dom.firstChild.getBoundingClientRect();
        evt.clientX = event.clientX;
        evt.clientY = event.clientY;
        evt.clientX = Math.min(Math.max(event.clientX, dropPointBoundingBox.left), dropPointBoundingBox.left + dropPointBoundingBox.width);
        evt.clientY = Math.min(Math.max(event.clientY, dropPointBoundingBox.top), dropPointBoundingBox.top + dropPointBoundingBox.height);
    ...
    destroy() {
        ...
        this.pmView.root.removeEventListener("dragstart", this.onDragStart);
        this.pmView.root.removeEventListener("dragover", this.onDragOver);
        this.pmView.root.removeEventListener("drop", this.onDrop, true);
        this.pmView.root.removeEventListener("dragend", this.onDragEnd, true);
...
export class SideMenuProsemirrorPlugin extends BlockNoteExtension {
    editor;
    static key() {
        return "sideMenu";
    }

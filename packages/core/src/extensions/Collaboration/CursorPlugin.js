...
export class CursorPlugin extends BlockNoteExtension {
    ...
    constructor(collaboration) {
        ...
        if (collaboration.showCursorLabels !== "always") {
            this.provider.awareness.on("change", ({ updated, }) => {
        ...
        this.addProsemirrorPlugin(yCursorPlugin(this.provider.awareness, {
    ...
    renderCursor = (user, clientID) => {
        let cursorData = this.recentlyUpdatedCursors.get(clientID);
        if (!cursorData) {
            const cursorElement = (this.collaboration.renderCursor ?? CursorPlugin.defaultCursorRender)(user);
    ...
    static defaultCursorRender = (user) => {
        ...
        caretElement.setAttribute("style", `background-color: ${user.color}; color: ${CursorPlugin.isDarkColor(user.color) ? "white" : "black"}`);
        const labelElement = document.createElement("span");
        labelElement.classList.add("bn-collaboration-cursor__label");
        labelElement.setAttribute("style", `background-color: ${user.color}; color: ${CursorPlugin.isDarkColor(user.color) ? "white" : "black"}`);

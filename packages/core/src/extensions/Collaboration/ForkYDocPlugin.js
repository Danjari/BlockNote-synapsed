...
import { yCursorPluginKey, ySyncPluginKey, yUndoPluginKey, } from "y-prosemirror";
...
export class ForkYDocPlugin extends BlockNoteExtension {
     ...
     */
    findTypeInOtherYdoc(ytype, otherYdoc) {
        ...
        if (ytype._item === null) {
             ...
             */
            const rootKey = Array.from(ydoc.share.keys()).find((key) => ydoc.share.get(key) === ytype);
            if (rootKey == null) {
                throw new Error("type does not exist in other ydoc");
            }
            return otherYdoc.get(rootKey, ytype.constructor);
        }
        else {
            /**
            ...
            const otherItem = otherStructs[itemIndex];
            const otherContent = otherItem.content;
            return otherContent.type;
        }
    }
    /**
    ...
    fork() {
        ...
        this.editor._tiptapEditor.registerPlugin(new SyncPlugin(forkedFragment).plugins[0]);
        this.editor._tiptapEditor.registerPlugin(new UndoPlugin({ editor: this.editor }).plugins[0]);
    ...
    merge({ keepChanges }) {
        ...
        this.editor.extensions["yCursorPlugin"] = new CursorPlugin(this.collaboration);
        this.editor.extensions["yUndoPlugin"] = new UndoPlugin({
            editor: this.editor,
        });
        // Register the plugins again, based on the original fragment
        this.editor._tiptapEditor.registerPlugin(this.editor.extensions["ySyncPlugin"].plugins[0]);
        this.editor._tiptapEditor.registerPlugin(this.editor.extensions["yCursorPlugin"].plugins[0]);
        this.editor._tiptapEditor.registerPlugin(this.editor.extensions["yUndoPlugin"].plugins[0]);
        // Reset the undo stack to the original undo stack
        yUndoPluginKey.getState(this.editor.prosemirrorState).undoManager.undoStack = undoStack;
        if (keepChanges) {
            // Apply any changes that have been made to the fork, onto the original doc
            const update = Y.encodeStateAsUpdate(forkedFragment.doc, Y.encodeStateVector(originalFragment.doc));
...
//# sourceMappingURL=ForkYDocPlugin.js.map
...
import { CellSelection, addColumnAfter, addColumnBefore, addRowAfter, addRowBefore, deleteColumn, deleteRow, mergeCells, splitCell, } from "prosemirror-tables";
import { Decoration, DecorationSet } from "prosemirror-view";
import { addRowsOrColumns, areInSameColumn, canColumnBeDraggedInto, canRowBeDraggedInto, cropEmptyRowsOrColumns, getCellsAtColumnHandle, getCellsAtRowHandle, getDimensionsOfTable, moveColumn, moveRow, } from "../../api/blockManipulation/tables/tables.js";
import { nodeToBlock } from "../../api/nodeConversions/nodeToBlock.js";
import { getNodeById } from "../../api/nodeUtil.js";
import { checkBlockIsDefaultType, isTableCellSelection, } from "../../blocks/defaultBlockTypeGuards.js";
...
export class TableHandlesView {
    ...
    mouseMoveHandler = (event) => {
        ...
        let tableBlock;
        const pmNodeInfo = this.editor.transact((tr) => getNodeById(blockEl.id, tr.doc));
        if (!pmNodeInfo) {
            throw new Error(`Block with ID ${blockEl.id} not found`);
        }
        const block = nodeToBlock(pmNodeInfo.node, this.editor.pmSchema, this.editor.schema.blockSchema, this.editor.schema.inlineContentSchema, this.editor.schema.styleSchema);
    ...
    dropHandler = (event) => {
        ...
        if (draggingState.draggedCellOrientation === "row") {
            if (!canRowBeDraggedInto(this.state.block, draggingState.originalIndex, rowIndex)) {
                // If the target row is invalid, don't move the row
                return false;
            }
            const newTable = moveRow(this.state.block, draggingState.originalIndex, rowIndex);
            ...
            if (!canColumnBeDraggedInto(this.state.block, draggingState.originalIndex, colIndex)) {
                // If the target column is invalid, don't move the column
                return false;
            }
            const newTable = moveColumn(this.state.block, draggingState.originalIndex, colIndex);
    ...
    update() {
        ...
        const { height: rowCount, width: colCount } = getDimensionsOfTable(this.state.block);
        if (this.state.rowIndex !== undefined &&
            this.state.colIndex !== undefined) {
...
export class TableHandlesProsemirrorPlugin extends BlockNoteExtension {
    editor;
    static key() {
        return "tableHandles";
    }
            ...
            props: {
                decorations: (state) => {
                    ...
                    if (this.view.state.draggingState.draggedCellOrientation === "row") {
                        const cellsInRow = getCellsAtRowHandle(this.view.state.block, newIndex);
                        cellsInRow.forEach(({ row, col }) => {
                            // Gets each row in the table.
                            const rowResolvedPos = state.doc.resolve(tableResolvedPos.posAtIndex(row) + 1);
                            // Gets the cell within the row.
                            const cellResolvedPos = state.doc.resolve(rowResolvedPos.posAtIndex(col) + 1);
                        ...
                        const cellsInColumn = getCellsAtColumnHandle(this.view.state.block, newIndex);
    ...
    colDragStart = (event) => {
        ...
        this.editor.transact((tr) => tr.setMeta(tableHandlesPluginKey, {
     ...
     */
    setCellSelection = (state, relativeStartCell, relativeEndCell = relativeStartCell) => {
        const view = this.view;
        if (!view) {
            throw new Error("Table handles view not initialized");
        }
        const tableResolvedPos = state.doc.resolve(view.tablePos + 1);
        const startRowResolvedPos = state.doc.resolve(tableResolvedPos.posAtIndex(relativeStartCell.row) + 1);
        const startCellResolvedPos = state.doc.resolve(
        // No need for +1, since CellSelection expects the position before the cell
        startRowResolvedPos.posAtIndex(relativeStartCell.col));
        const endRowResolvedPos = state.doc.resolve(tableResolvedPos.posAtIndex(relativeEndCell.row) + 1);
        ...
        tr.setSelection(new CellSelection(startCellResolvedPos, endCellResolvedPos));
    ...
    addRowOrColumn = (index, direction) => {
        this.editor.exec((beforeState, dispatch) => {
            const state = this.setCellSelection(beforeState, direction.orientation === "row"
                ? { row: index, col: 0 }
                : { row: 0, col: index });
    ...
    mergeCells = (cellsToMerge) => {
        return this.editor.exec((beforeState, dispatch) => {
            const state = cellsToMerge
                ? this.setCellSelection(beforeState, cellsToMerge.relativeStartCell, cellsToMerge.relativeEndCell)
                : beforeState;
    ...
    getCellSelection = () => {
        ...
        return this.editor.transact((tr) => {
            ...
            if (isTableCellSelection(selection)) {
                ...
                $fromCell = tr.doc.resolve(selection.$from.pos - selection.$from.parentOffset - 1);
                $toCell = tr.doc.resolve(selection.$to.pos - selection.$to.parentOffset - 1);
            ...
            const $fromRow = tr.doc.resolve($fromCell.pos - $fromCell.parentOffset - 1);

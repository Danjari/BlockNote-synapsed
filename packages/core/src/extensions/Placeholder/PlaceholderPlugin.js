    ...
    constructor(editor, placeholders) {
        ...
        this.addProsemirrorPlugin(new Plugin({
            ...
            view: (view) => {
                ...
                const getSelector = (additionalSelectors = "") => `.${uniqueEditorSelector} .bn-block-content${additionalSelectors} .bn-inline-content:has(> .ProseMirror-trailingBreak:only-child):before`;
                try {
                    // FIXME: the names "default" and "emptyDocument" are hardcoded
                    const { default: defaultPlaceholder, emptyDocument: emptyPlaceholder, ...rest } = placeholders;
                    // add block specific placeholders
                    for (const [blockType, placeholder] of Object.entries(rest)) {
                        const blockTypeSelector = `[data-content-type="${blockType}"]`;
                        styleSheet.insertRule(`${getSelector(blockTypeSelector)} { content: ${JSON.stringify(placeholder)}; }`);
                    }
                    ...
                    styleSheet.insertRule(`${getSelector(onlyBlockSelector)} { content: ${JSON.stringify(emptyPlaceholder)}; }`);

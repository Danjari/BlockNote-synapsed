    ...
    addKeyboardShortcuts() {
        ...
        const handleBackspace = () => this.editor.commands.first(({ chain, commands }) => [
            ...
            () => commands.command(({ state }) => {
                ...
                if (selectionAtBlockStart && !isParagraph) {
                    return commands.command(updateBlockCommand(blockInfo.bnBlock.beforePos, {
                        type: "paragraph",
                        props: {},
                    }));
                }
            ...
            () => commands.command(({ state }) => {
                ...
                if (blockEmpty) {
                    ...
                    if (prevBlockInfo.blockContent.node.type.spec.content ===
                        "tableRow+") {
                        ...
                        chainedCommands = chainedCommands.setTextSelection(lastCellParagraphEndPos);
                    }
                    else if (prevBlockInfo.blockContent.node.type.spec.content === "") {
                        ...
                        const blockContentStartPos = prevBlockInfo.blockContent.afterPos -
                            prevBlockInfo.blockContent.node.nodeSize;
                        chainedCommands =
        ...
        const handleEnter = (withShift = false) => {
            return this.editor.commands.first(({ commands, tr }) => [
                ...
                () => commands.command(({ state }) => {
                    const blockInfo = getBlockInfoFromSelection(state);
                    const blockHardBreakShortcut = this.options.editor.schema.blockSchema[blockInfo.blockNoteType]
                    ...
                    (blockHardBreakShortcut === "shift+enter" && withShift) ||
                        ...
                        const marks = tr.storedMarks ||
                            tr.selection.$head
                                .marks()
                                .filter((m) => this.editor.extensionManager.splittableMarks.includes(m.type.name));
                        tr.insert(tr.selection.head, tr.doc.type.schema.nodes.hardBreak.create()).ensureMarks(marks);
                        return true;
                    }
                    return false;
                }),

...
export function commentToYMap(comment) {
...
export function threadToYMap(thread) {
    const yMap = new Y.Map();
...
export function yMapToReaction(yMap) {
    return {
...
function yMapToReactions(yMap) {
    const flatReactions = [...yMap.values()].map((reaction) => yMapToReaction(reaction));
    // combine reactions by the same emoji
    return flatReactions.reduce((acc, reaction) => {
        const existingReaction = acc.find((r) => r.emoji === reaction.emoji);
        if (existingReaction) {
            existingReaction.userIds.push(reaction.userId);
            existingReaction.createdAt = new Date(Math.min(existingReaction.createdAt.getTime(), reaction.createdAt.getTime()));
        }
        else {
            acc.push({
...
export function yMapToComment(yMap) {
    return {
        type: "comment",
        id: yMap.get("id"),
        userId: yMap.get("userId"),
...
export function yMapToThread(yMap) {
    return {
        type: "thread",
        id: yMap.get("id"),
        createdAt: new Date(yMap.get("createdAt")),
        updatedAt: new Date(yMap.get("updatedAt")),
        comments: (yMap.get("comments") || []).map((comment) => yMapToComment(comment)),
...
//# sourceMappingURL=yjsHelpers.js.map
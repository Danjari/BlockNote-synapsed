...
export const shikiHighlighterPromiseSymbol = Symbol.for("blocknote.shikiHighlighterPromise");
...
const CodeBlockContent = createStronglyTypedTiptapNode({
    ...
    addNodeView() {
        ...
        return ({ editor, node, getPos, HTMLAttributes }) => {
            ...
            Object.entries(options.editor.settings.codeBlock.supportedLanguages).forEach(([id, { name }]) => {
        ...
        const lazyParser = (parserOptions) => {
            if (!options.editor.settings.codeBlock.createHighlighter) {
                if (process.env.NODE_ENV === "development" && !hasWarned) {
                    ...
                    console.log("For syntax highlighting of code blocks, you must provide a `codeBlock.createHighlighter` function");
                    hasWarned = true;
                }
                return [];
            }
            if (!highlighter) {
                ...
                return globalThisForShiki[shikiHighlighterPromiseSymbol].then((createdHighlighter) => {
                    highlighter = createdHighlighter;
                });
            }
            const language = getLanguageId(options.editor.settings.codeBlock, parserOptions.language);
            if (!language ||
                language === "text" ||
                language === "none" ||
                language === "plaintext" ||
        ...
        return [
            new InputRule({
                ...
                handler: ({ state, range, match }) => {
                    ...
                    const attributes = {
                        language: getLanguageId(options.editor.settings.codeBlock, languageName) ??
                            languageName,
                    };
                    if (!$start
                        .node(-1)

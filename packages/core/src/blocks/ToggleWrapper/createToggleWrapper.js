export const defaultToggledState = {
    set: (block, isToggled) => window.localStorage.setItem(`toggle-${block.id}`, isToggled ? "true" : "false"),
    get: (block) => window.localStorage.getItem(`toggle-${block.id}`) === "true",
};
export const createToggleWrapper = (block, editor, renderedElement, toggledState = defaultToggledState) => {
    ...
    const toggleButtonMouseDown = (event) => event.preventDefault();
    ...
    const toggleAddBlockButtonMouseDown = (event) => event.preventDefault();
    toggleAddBlockButton.addEventListener("mousedown", toggleAddBlockButtonMouseDown);
    ...
    return {
        ...
        destroy: () => {
            ...
            toggleAddBlockButton.removeEventListener("mousedown", toggleAddBlockButtonMouseDown);
            toggleAddBlockButton.removeEventListener("click", toggleAddBlockButtonOnClick);
            onEditorChange?.();
        },
    };
};
//# sourceMappingURL=createToggleWrapper.js.map
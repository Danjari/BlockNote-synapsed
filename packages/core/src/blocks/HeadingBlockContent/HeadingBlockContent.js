...
import { createBlockSpecFromStronglyTypedTiptapNode, createStronglyTypedTiptapNode, getBlockFromPos, propsToAttributes, } from "../../schema/index.js";
...
const HeadingBlockContent = createStronglyTypedTiptapNode({
            ...
            ...editor.settings.heading.levels.map((level) => {
                ...
                return new InputRule({
                    ...
                    handler: ({ state, chain, range }) => {
                        ...
                        chain()
                            .command(updateBlockCommand(blockInfo.bnBlock.beforePos, {
                            type: "heading",
                            props: {
                                level: level,
                            },
    ...
    addKeyboardShortcuts() {
        const editor = this.options.editor;
        return Object.fromEntries(editor.settings.heading.levels.map((level) => [
            `Mod-Alt-${level}`,
            () => {
    ...
    addNodeView() {
        return ({ node, HTMLAttributes, getPos }) => {
            const { dom, contentDOM } = createDefaultBlockDOMOutputSpec(this.name, `h${node.attrs.level}`, {
                ...(this.options.domAttributes?.blockContent || {}),
                ...HTMLAttributes,
            }, this.options.domAttributes?.inlineContent || {});
            ...
            const toggleWrapper = createToggleWrapper(block, editor, contentDOM);

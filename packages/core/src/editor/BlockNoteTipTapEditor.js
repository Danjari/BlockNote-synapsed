import { Editor as TiptapEditor, createDocument, } from "@tiptap/core";
...
export class BlockNoteTipTapEditor extends TiptapEditor {
    ...
    dispatch(transaction) {
        ...
        const { state, transactions: appendedTransactions } = this.state.applyTransaction(transaction);
    ...
    forceEnablePlugins() {
        if (this.view) {
            throw new Error("forcePluginsEnabled called after view has been initialized");
        }
     ...
     */
    createViewAlternative(blockNoteEditor, contentComponent) {
        ...
        this.commands.focus(this.options.autofocus ||
            this.options.element.getAttribute("data-bn-autofocus") === "true", { scrollIntoView: false });
     ...
     */
    mount = (blockNoteEditor, element, contentComponent) => {
        if (!element) {
            this.destroy();
